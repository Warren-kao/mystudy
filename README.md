...
· C 语言基础：任何支持 C99 的编译器（GCC、Clang、MSVC），推荐使用 Code::Blocks 或 VS Code
...
1. C 语言基础练习
      进入 C_Basics/ 目录，使用 gcc 编译运行：
      gcc example.c -o example && ./example
2. C51 单片机例程
      进入 C51/ 对应子目录，用 Keil C51 打开 .uvproj 工程，编译后下载到 C51 开发板（如 STC89C52）。
3. STM32 裸机例程
      进入 STM32/ 对应例程目录，使用 Keil MDK 打开 .uvprojx 工程，编译并下载至目标板。
4. FreeRTOS 例程
      进入 FreeRTOS/ 目录，基于 STM32 工程，通过串口打印任务运行日志，观察任务调度。
...
```
mystudy/
├── C_Basics/               # C 语言基础语法、指针、内存管理、算法练习
├── C51/                    # C51 单片机外设例程（LED、数码管、按键、中断、串口等）
├── STM32/                  # STM32 裸机例程（标准库/HAL库）
├── FreeRTOS/               # FreeRTOS 移植与应用（任务、队列、信号量等）
...
· Keil Example - 官方例程参考
· STM32CubeF1 - STM32 官方 HAL 库
· FreeRTOS-Kernel - FreeRTOS 官方内核
· STC-ISP - C51 烧录工具
...
这个版本明确划分了 C 语言基础、C51、STM32、FreeRTOS 的目录，并分别说明了开发环境和用法，更贴合您要存放的代码类型。您可以根据实际情况调整目录名称或例程细节。
